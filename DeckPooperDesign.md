...
部族或主题工具包是人工筛选后的“可能相关牌集合”，不是整套必选牌表，也不是最终候选池。每次系列、补充产品或牌面规则更新后，必须先按新快照复核并更新工具包（新增、移除、合法性、平台印刷和角色备注），再从工具包中选取本次可能用上的部分进入主题种子或候选池。构筑器只消费这部分输入，不把工具包中的未选牌自动塞入最终套牌。
...
- **`draft_core.py` 平移进 `deck_core.py` 后退役**：定型内核的 36 例单测保持绿色；不留双份实现，测试随同迁移。
